...
export default function RezmeAdminDashboard() {
  ...
  const [isHRAdminsTableCollapsed, setIsHRAdminsTableCollapsed] = useState(false);
      ...
      if (data && data.length > 0) {
        const hrAdminsWithConnections = await Promise.all(
          data.map(async (admin) => {
            ...
            if (permError) {
              console.error("Error fetching permissions for admin:", admin.id, permError);
              return { 
                ...admin, 
                connected_users: [],
            ...
            if (connectedCount > 0) {
              ...
              invitedCandidatesCount = Math.floor(connectedCount * inviteMultiplier);
        ...
        setHRAdmins(data?.map(admin => {
          ...
          const addressIndex = admin.id.charCodeAt(0) % placeholderAddresses.length;
          const phoneIndex = admin.id.charCodeAt(1) % placeholderPhones.length;
  ...
  const getConnectedHRAdmins = (userId: string) => {
    ...
    const connected = hrAdmins.filter(admin => 
      admin.connected_users && admin.connected_users.includes(userId)
    );
  ...
  const filteredUsers = users.filter((user) => {
    ...
    return fullName.includes(search) || email.includes(search) || phone.includes(search) || city.includes(search);
  });
  ...
  const filteredHRAdmins = hrAdmins.filter((admin) => {
    ...
    return fullName.includes(search) || email.includes(search) || company.includes(search) || phone.includes(search);
  });
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div className="p-4 border border-gray-200 rounded-lg">
                    <h4 className="text-lg font-semibold text-black mb-3" style={{ fontFamily: 'Poppins, sans-serif' }}>
                      Personal Information
                    </h4>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                          ...
                          {selectedUserDetails.rr_completed ? "Completed" : "In Progress"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mb-6">
                  <div className="p-4 border border-gray-200 rounded-lg">
                    ...
                    {(() => {
                      const connectedAdmins = getConnectedHRAdmins(selectedUserDetails.id);
                        ...
                        return (
                          <div className="text-center py-4">
                            ...
                            <p className="text-xs text-gray-400 mt-1" style={{ fontFamily: 'Poppins, sans-serif' }}>
                              This candidate has not been connected to any HR administrators
                            </p>
                          </div>
                        );
                      }
                      ...
                      return (
                        <div className="space-y-3">
                          {connectedAdmins.map((admin, index) => {
                            ...
                            const displayName = adminName === "company company" || adminName === "company" 
                              ? admin.company 
                              : adminName;
                            ...
                            return (
                              <div key={admin.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div className="flex items-center space-x-3">
                                  ...
                                  <div>
                                    <p className="font-medium text-black" style={{ fontFamily: 'Poppins, sans-serif' }}>
                                      {displayName}
                                    </p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  aria-label={isUsersTableCollapsed ? "Expand table" : "Collapse table"}
                >
                  <svg 
                    className={`w-4 h-4 text-gray-600 transform transition-transform duration-200 ${isUsersTableCollapsed ? 'rotate-180' : ''}`}
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
            ...
            {!isUsersTableCollapsed && searchTerm && (
              <div className="mt-3">
                <p className="text-sm text-gray-600" style={{ fontFamily: 'Poppins, sans-serif' }}>
                  ...
                  {searchTerm && (
                    <span>
                      {" "}for "<span className="font-medium text-red-600">{searchTerm}</span>"
                    </span>
                  )}
                </p>
              </div>
                  ...
                  {filteredUsers.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-4 py-8 text-center">
                        <div className="text-gray-500" style={{ fontFamily: 'Poppins, sans-serif' }}>
                          {searchTerm ? (
                            <div>
                    ...
                    {user.email}
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <div>
                            {(() => {
                              const connectedAdmins = getConnectedHRAdmins(user.id);
                              if (connectedAdmins.length === 0) {
                                return (
          ...
          {isUsersTableCollapsed && (
            <div className="px-6 py-8 text-center">
              <div className="text-gray-500" style={{ fontFamily: 'Poppins, sans-serif' }}>
                <p className="text-sm">Table collapsed - Click the arrow above to expand</p>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-4 max-w-4xl mx-auto">
                  ...
                  <div className="text-center">
                    <p className="text-lg font-semibold text-green-600">{users.filter(u => u.is_visible_to_hr).length}</p>
                    <p className="text-xs text-gray-600">HR Visible</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-semibold text-blue-600">{users.filter(u => u.rr_completed).length}</p>
                    <p className="text-xs text-gray-600">RR Complete</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-semibold text-green-600">{users.filter(u => u.wotc_signature_date).length}</p>
                    <p className="text-xs text-gray-600">WOTC Complete</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-semibold text-green-600">
                      {hrAdmins.filter(admin => admin.payment_plan === 'Pro' || admin.payment_plan === 'Premium').length}
                    </p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                  aria-label={isHRAdminsTableCollapsed ? "Expand table" : "Collapse table"}
                >
                  <svg 
                    className={`w-4 h-4 text-gray-600 transform transition-transform duration-200 ${isHRAdminsTableCollapsed ? 'rotate-180' : ''}`}
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
            ...
            {!isHRAdminsTableCollapsed && hrSearchTerm && (
              <div className="mt-3">
                <p className="text-sm text-gray-600" style={{ fontFamily: 'Poppins, sans-serif' }}>
                  Showing {filteredHRAdmins.length} of {hrAdmins.length} HR admins
                  {hrSearchTerm && (
                    <span>
                      {" "}for "<span className="font-medium text-red-600">{hrSearchTerm}</span>"
                    </span>
                  )}
                </p>
              </div>
                  ...
                  {filteredHRAdmins.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="px-4 py-8 text-center">
                        <div className="text-gray-500" style={{ fontFamily: 'Poppins, sans-serif' }}>
                          {hrSearchTerm ? (
                            <div>
                              <p className="text-sm">No HR admins found matching your search.</p>
                              <p className="text-xs mt-1">Try adjusting your search terms.</p>
                            </div>
                          ) : (
                    ...
                    {admin.company}
                        ...
                        <td 
                          ...
                          style={{ fontFamily: 'Poppins, sans-serif' }}
                        >
                          <div className="max-w-32 truncate" title={admin.company_address}>
                        ...
                        <td 
                          ...
                          style={{ fontFamily: 'Poppins, sans-serif' }}
                        >
                          <div className="max-w-24 truncate" title={admin.phone_number}>
                            {admin.phone_number}
                          </div>
                        </td>
                        <td 
                        ...
                        >
                          {admin.payment_plan ? (
                            ...
                            >
                              {admin.payment_plan === 'Premium' ? 'Prem' : admin.payment_plan}
                            </span>
                          ) : (
          ...
          {isHRAdminsTableCollapsed && (
            <div className="px-6 py-8 text-center">
              <div className="text-gray-500" style={{ fontFamily: 'Poppins, sans-serif' }}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-4 max-w-4xl mx-auto">
                  <div className="text-center">
                    <p className="text-lg font-semibold text-black">{hrAdmins.length}</p>
                    <p className="text-xs text-gray-600">Total HR Admins</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-semibold text-blue-600">
                      {hrAdmins.reduce((total, admin) => total + (admin.connected_users?.length || 0), 0)}
                    </p>
                    <p className="text-xs text-gray-600">Total Connections</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-semibold text-orange-600">
                      {hrAdmins.reduce((total, admin) => total + (admin.invited_candidates_count || 0), 0)}
                    </p>
                    <p className="text-xs text-gray-600">Total Invites</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-semibold text-purple-600">
                      {hrAdmins.reduce((total, admin) => total + (admin.assessments_completed || 0), 0)}
                    </p>
